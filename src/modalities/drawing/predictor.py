...
class DrawingPredictor:
    ...
    def _preprocess(pil_img: Image.Image, img_size: tuple, noise_std: float = 0.03) -> np.ndarray:
        """Grayscale → resize → invert → bruit gaussien (simule texture papier des données d'entraînement)."""
        arr = np.array(pil_img.convert("L").resize(img_size, Image.LANCZOS), dtype=np.float32) / 255.0
        arr = 1.0 - arr
        # Bruit gaussien léger pour réduire le domain shift (entraînement = photos JPEG, inférence = PNG propre)
    ...
    def predict(self, payload: dict[str, Any]) -> PredictionResult:
        ...
        if not self.model_path.exists():
            return PredictionResult(
                ...
                warnings=["Modèle dessin introuvable. Exécutez notebooks/drawing_model_training.ipynb."],
            )
